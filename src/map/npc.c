...
/**
 * Executed via map->foreachinrange, verifies if any NPCs are present
 * @lock db_lock(npc->name_db)
 **/
static int npc_isnear_sub(struct block_list *bl, va_list args)
{
	...
	nd = npc->bl2nd_nodblock(bl);

	// Ignore hidden / invisible / fake npcs
	if(nd && !(nd->option & (OPTION_HIDE|OPTION_INVISIBLE))) {
		if(!(battle_config.vendchat_near_hiddennpc
			&& ( nd->class_ == FAKE_NPC || nd->class_ == HIDDEN_WARP_CLASS ))
		)
			return 1;
	}
	return 0;
...
/**
 * Verifies if given bl is near an NPC
 **/
static bool npc_isnear(struct block_list *bl)
{
	...
	bool in_range = map->foreachinrange(npc_isnear_sub, bl,
		battle_config.min_npc_vendchat_distance, BL_NPC);
	npc->unlock_name_db();
	return in_range;
}
...
/**
 * Runs OnTouch (see script->config.ontouch_name)
 * This function is called from checks if the player (sd) is in valid touch area (nd)
 *
 * @return 0 No OnTouch available in this NPC
 * @return 1 Local OnTouch
 * @return 2 AreaNPC
 * @lock npc_data
 **/
...
/**
 * Sub-function of npc_enable, called via map->foreachinarea
 * Checks if any of the bl's in the area are of BL_PC type and if if they're in
 * proper OnTouch/OnTouchArea range, if so tries to run the event.
 **/
static int npc_enable_sub_pc_touch(struct block_list *bl, va_list ap)
{
	struct npc_data *nd;
	...
	if (bl->type == BL_PC) {
		...
		if( npc_ontouch_event(sd,nd) > 0 && npc->ontouch2_event(sd,nd) > 0 )
...
/**
 * Enables an NPC identified by the given name, updates NPC option.
 * When flag is OPTION_NOTHING the NPC is effectively disabled.
 *
 * @param name    NPC unique name
 * @param namelen Length of NPC unique name, if 0 it's calculated internally
 * @param flag    Enable flag (@see OPTION_)
 * @return BOOL Success
 **/
static bool npc_enable(const char *name, int namelen, int flag)
{
	struct npc_data* nd = npc->name2id(name, 0);
	...
	if( flag&3 && (nd->u.scr.xs >= 0 || nd->u.scr.ys >= 0) ) //check if player standing on a OnTouchArea
		map->foreachinarea( npc_enable_sub_pc_touch, nd->bl.m, nd->bl.x-nd->u.scr.xs,
			nd->bl.y-nd->u.scr.ys, nd->bl.x+nd->u.scr.xs, nd->bl.y+nd->u.scr.ys,
			BL_PC, nd );
 ...
 * Unlocks NPC name db
 **/
static void npc_unlock_name_db(void)
{
	db_unlock(npc->name_db);
}

/**
 * Locks NPC name db
 **/
static void npc_lock_name_db(enum lock_type lt)
{
	db_lock(npc->name_db, lt);
}

/**
 * Unlocks previously locked NPC data
 *
 * @see npc_name2id
 * @see npc->name_db
 **/
static void npc_unlock_data(struct npc_data *nd)
{
#ifdef NPC_DEBUG_MULTI_THREAD
	int prev_tid;
	if((prev_tid = InterlockedCompareExchange(&nd->thread_owner, -1, thread->get_tid()))
		!= thread->get_tid()
	) {
		ShowError("npc_unlock_data: improper lock of npc data occurred, NPC '%s'"
			      " thread id %d, prev %d\n",
				  nd->name, thread->get_tid(), prev_tid);
		nd->thread_owner = -1; // This is a hack so we can try to restore from this failed state
	}
#endif
	mutex->unlock(nd->lock);
	if(nd->name_db_locked)
		db_unlock(npc->name_db);
	nd->name_db_locked = false;
}

/**
 * (Private) Locks an NPC data object.
 * @return bool Success
 **/
static bool npc_lock_data(struct npc_data *nd)
{
#ifdef NPC_DEBUG_MULTI_THREAD
	if(InterlockedAdd(nd->thread_owner, 0) == thread->get_tid()) {
		ShowError("npc_name2id: Thread %d is trying to reacquire npc data mutex for NPC(%d) '%s'\n"
		          "npc_name2id: Aborting operation.\n",
		          thread->get_tid(), nd->bl.id, nd->exname);
		return false;
	}
#endif
	mutex->lock(nd->lock);
...
#ifdef NPC_DEBUG_MULTI_THREAD
	int prev_tid;
	if((prev_tid = InterlockedCompareExchange(&nd->thread_owner, thread->get_tid(), -1)) != -1) {
		ShowError("npc_name2id: improper unlock of npc data occurred, NPC(%d) '%s'"
				  " thread id %d, prev %d\n",
				  nd->bl.id, nd->exname, thread->get_tid(), prev_tid);
		// This is a hack so we can try to restore from this failed state
		nd->thread_owner = -1;
		mutex->unlock(nd->lock);
		return false;
	}
...
/**
 * Converts a block list object to a NPC data object
 * This doesn't acquire db_lock(npc->name_db), so it must be locked before calling, this
 * function is supposed to be used when multiple NPC bls are going to be accessed in
 * quick succession so we can avoid multiple lock-unlocks of a db_lock.
 * This function must be paired with a release function.
 * npc->name_db can be locked via npc->lock_name_db
 * @see npc_unlock_data
 **/
static struct npc_data *npc_bl2nd_nodblock(struct block_list *bl)
{
	if(!bl)
		return NULL;
	struct npc_data *nd = BL_CAST(BL_NPC, bl);
	if(!nd || !npc_lock_data(nd))
		return NULL;
	nd->name_db_locked = false;
	return nd;
}
...
/**
 * Converts a block list object to a NPC data object
 * This acquires both db_lock(npc->name_db) and also nd->lock, all calls
 * to this function must be paired with a release function.
 * @see npc_unlock_data
 **/
static struct npc_data *npc_bl2nd(struct block_list *bl)
{
	if(!bl)
		return NULL;
	// npc->name_db is locked to prevent removal of the NPC name while it's
	// being accessed.
	db_lock(npc->name_db, READ_LOCK);
	struct npc_data *nd = BL_CAST(BL_NPC, bl);
	if(!nd || !npc_lock_data(nd)) {
		db_unlock(npc->name_db);
		return NULL;
	}
	nd->name_db_locked = true;
	return nd;
}
 ...
 * @see npc_unlock_data
 **/
static struct npc_data *npc_id2nd(int id)
{
 ...
 * This acquires both db_lock(npc->name_db) and also nd->lock, all calls
 * to this function must be paired with a release function.
 * @param name_len Length of name, if 0 length is calculated internally
 * @see npc->name_db
 * @see npc_unlock_data
 **/
static struct npc_data *npc_name2id(const char *name, int name_len)
{
	db_lock(npc->name_db, READ_LOCK);
	struct npc_data *nd = strdb_get(npc->name_db, name, name_len);
	if(!nd || !npc_lock_data(nd)) {
		db_unlock(npc->name_db);
		return NULL;
	}
...
/**
 * Finds if this name is of a valid NPC
 * Acquires db_lock(npc->name_db)
 * @return bool
 **/
bool npc_name_is_valid(const char *name, int name_len)
{
	db_lock(npc->name_db, READ_LOCK);
	bool retv = (strdb_get(npc->name_db, name, name_len) != NULL);
	db_unlock(npc->name_db);
	return retv;
}
 ...
 *
 * Acquires NPC data lock (nd->lock)
 **/
 ...
 * @return 0 Success
 * Acquires db_lock(npc->ev_db) and also NPC data lock (nd->lock)
 **/
...
static int npc_touchnext_areanpc(struct map_session_data *sd, bool leavemap)
{
	...
	nullpo_retr(1, sd);
	nd = npc->id2nd(sd->touching_id);
	if(!nd)
		return 1;
	if(nd->touching_id != sd->bl.id ) {
		npc->unlock_data(nd);
		return 1;
	}
...
static int npc_touch_areanpc(struct map_session_data *sd, int16 m, int16 x, int16 y)
{
	...
	switch(map->list[m].npc[i]->subtype) {
		...
		case SCRIPT:
			...
			if( npc_ontouch_event(sd,map->list[m].npc[i]) > 0 && npc->ontouch2_event(sd,map->list[m].npc[i]) > 0 )
			{ // failed to run OnTouch event, so just click the npc
...
/**
 * Checks if player is not too far to access the NPC
 * @return bool Success (player is near enough)
 * @lock npc_data
 **/
static bool npc_checknear(struct map_session_data *sd, struct npc_data *nd)
{
...
static int npc_scriptcont(struct map_session_data *sd, int id, bool closing)
{
	...
	struct npc_data *target_nd = npc->id2nd(id);
	if(!target_nd)
		return 1;
...
#endif
		if (id != sd->npc_id) {
			struct npc_data *nd_sd = npc->id2nd(sd->npc_id);
			ShowDebug("npc_scriptcont: %s (sd->npc_id=%d) is not %s (id=%d).\n",
				(nd_sd != NULL) ? nd_sd->name : "'Unknown NPC'", sd->npc_id,
				target_nd->name, id);
			npc->unlock_data(nd_sd);
			npc->unlock_data(target_nd);
			return 1;
		}
#ifdef SECURE_NPCTIMEOUT
	}
	...
	if (id != npc->fake_nd->bl.id) { /// Not an item script.
		...
		} else if (npc->checknear(sd, target_nd) == NULL) {
...
/**
 * Processes and validates given buy list for a character
 * @param sd        Player character
 * @param item_list Player item list to be bought
 * @param nd        Target NPC
 * @param shop      Target NPC item list
 * @param shop_size Target NPC item list length
 * @param out_price Price to be charged (out)
 * @param coin      Quantity of coin available to use when buying (zeny, cash etc)
 * @param flag      Buy list allow flags (@see npc_buy_flags)
 **/
static enum npc_buy_result npc_generic_buylist_process_validate(struct map_session_data *sd,
...
)
{
	...
	for(int i = 0; i < VECTOR_LENGTH(*item_list); ++i) {
		...
		if(j == shop_size || shop[j].value <= 0)
			return NPCBL_ITEM_NOT_FOUND_SHOP;

		entry->id = shop[j].nameid; //item_avail replacement
		value = shop[j].value;

		if(flag&BUY_ENFORCE_MAXIMUM_AMOUNT && entry->amount > (int)shop[j].qty)
			return NPCBL_AMOUNT_TOO_BIG;

		if(!itemdb->exists(entry->id))
			return NPCBL_ITEM_NOT_FOUND_DB; // item no longer in itemdb

		if(!itemdb->isstackable(entry->id) && entry->amount > 1) {
			ShowWarning(
				"Player %s (%d:%d) sent a hexed packet trying to buy %d of "
				"non-stackable item %d!\n",
				sd->status.name, sd->status.account_id, sd->status.char_id,
				entry->amount, entry->id);
			entry->amount = 1;
		}

		if(flag&BUY_ALLOW_SCRIPT_MASTER && nd->master_nd) {
			// Script-controlled shops decide by themselves, what can be bought and for what price.
			// Remaining checks are done in-script
			continue;
		}
		...
		switch (pc->checkadditem(sd, entry->id, entry->amount)) {
			...
			case ADDITEM_OVERAMOUNT:
				// Passed maximum possible amount
				return NPCBL_OVER_WEIGHT;
		}

		if(flag&BUY_ALLOW_DISCOUNT)
			value = pc->modifybuyvalue(sd,value);

		if((int64)value * entry->amount > INT_MAX) {
			ShowWarning("npc_generic_buylist_process_validate: Item (%d) price overflow attempt!\n",
				entry->id);
		...
		if(z > INT_MAX) {
			ShowWarning("npc_generic_buylist_process_validate: Item list price overflow attempt!\n");
			ShowDebug("(NPC:'%s' (%s,%d,%d), player:'%s' (%d/%d), value:%u, amount:%d)\n",
					nd->exname, map->list[nd->bl.m].name, nd->bl.x, nd->bl.y,
					sd->status.name, sd->status.account_id, sd->status.char_id,
					value, entry->amount);
	...
	// Script-controlled shops decide by themselves, what can be bought and for what price.
	if(flag&BUY_ALLOW_SCRIPT_MASTER && nd->master_nd != NULL)
		return NPCBL_SUCCESS;

	if (!(flag&BUY_IGNORE_COIN_AMOUNT) && z > coin)
		return NPCBL_NO_COIN;
	if( w + sd->weight > sd->max_weight )
		return NPCBL_OVER_WEIGHT;
	if( pc->inventoryblank(sd) < new_ )
		return NPCBL_OUT_OF_SPACE;
...
/**
 * Checks if valid call then open buy or selling list
 * @param sd   The character's session data.
 * @param id   The NPC ID.
 * @param type 0 Buy list
 * @param type 1 Sell list
 * @return 0 Success
 **/
...
/**
 * Effectivates payment and delivery of items (subroutine of npc_cashshop_buylist)
 * @return enum cashshop_error
 **/
static int npc_cashshop_buylist_pay_deliver(struct map_session_data *sd, int points,
	int price, struct itemlist *item_list, struct npc_data *nd
)
{
	// Payment Process ----------------------------------------------------
...
static int npc_cashshop_buylist(struct map_session_data *sd, int points, struct itemlist *item_list)
{
	...
	int out_price = 0;
	enum npc_buylist_result validation;
	validation = npc->generic_buylist_process_validate(sd, item_list, nd, shop, shop_size,
		&out_price, points, BUY_IGNORE_COIN_AMOUNT);
	if(validation != NPCBL_SUCCESS) {
		npc->unlock_data(nd);
		switch(validation) {
		case NPCBL_ITEM_NOT_FOUND_SHOP:return ERROR_TYPE_ITEM_ID;
		case NPCBL_ITEM_NOT_FOUND_DB:  return ERROR_TYPE_ITEM_ID;
		case NPCBL_AMOUNT_TOO_BIG:	   return ERROR_TYPE_QUANTITY;
		case NPCBL_NO_COIN:			   return ERROR_TYPE_MONEY;
		case NPCBL_OVER_WEIGHT:		   return ERROR_TYPE_INVENTORY_WEIGHT;
		case NPCBL_OUT_OF_SPACE:	   return ERROR_TYPE_INVENTORY_WEIGHT;
		default:                       return ERROR_TYPE_SYSTEM;
		}
	}
	...
	int retval;
	retval = npc_cashshop_buylist_pay_deliver(sd, points, vt, item_list, nd);
	npc->unlock_data(nd);
	return retval;
}

/**
 * npc_buylist subroutine for script-controlled shops
 * Prepares script variables for buylist processing in script-controlled shops
 * @lock npc_data
 **/
...
static void npc_trader_update(int master)
{
	...
	struct npc_data *master_nd = npc->id2nd(master);
...
/**
 * Performs pay action on a cashshop (subroutine of npc_cashshop_buy)
 * @param sd     Player session data of buyer
 * @param nd     NPC data
 * @param price  Price to be deduced from points
 * @param points Current points
 * @return enum cashshop_error
 **/
static int npc_cashshop_buy_pay(struct map_session_data *sd, struct npc_data *nd, int price, int points)
{
	if( nd->subtype == SCRIPT && nd->u.scr.shop->type == NST_CUSTOM ) {
		if( !npc->trader_pay(nd,sd,price,points) )
			return ERROR_TYPE_MONEY;
...
/**
 * Validates buy action on a cashshop (subroutine of npc_cashshop_buy)
 * @param sd        Session data of buyer
 * @param nameid    Item id
 * @param amount    Item amount
 * @param points    [in/out]player points
 * @param nd        NPC data
 * @param out_price	Valued to be paid
 * @return enum cashshop_error
 **/
static int npc_cashshop_buy_validate(struct map_session_data *sd, int nameid, int amount, int *points,
	struct npc_data *nd, int *out_price
)
{
	struct npc_item_list *shop = NULL;
	unsigned short shop_size = 0;
...
static int npc_cashshop_buy(struct map_session_data *sd, int nameid, int amount, int points)
{
	...
	int retval;
	int price = 0;
	retval = npc_cashshop_buy_validate(sd, nameid, amount, &points, nd, &price);
	if(retval != ERROR_TYPE_NONE) {
		npc->unlock_data(nd);
		return retval;
	}

	retval = npc_cashshop_buy_pay(sd, nd, price, points);
	if(retval != ERROR_TYPE_NONE) {
		npc->unlock_data(nd);
		return retval;
	}
...
static int npc_buylist(struct map_session_data *sd, struct itemlist *item_list)
{
	...
	enum npc_buy_result validate_code = 
		npc->generic_buylist_process_validate(sd, item_list, nd, shop, shop_size,
			&out_price, sd->status.zeny, BUY_ALLOW_DISCOUNT|BUY_ALLOW_SCRIPT_MASTER);
	if(validate_code != NPCBL_SUCCESS) {
		npc->unlock_data(nd);
		switch(validate_code) {
		case NPCBL_ITEM_NOT_FOUND_SHOP: return 3;
		case NPCBL_ITEM_NOT_FOUND_DB:	return 3;
#if PACKETVER >= 20110705
		case NPCBL_AMOUNT_TOO_BIG:		return 9;
#else
		case NPCBL_AMOUNT_TOO_BIG:		return 2;
#endif
		case NPCBL_NO_COIN:				return 1;
		case NPCBL_OVER_WEIGHT:			return 2;
		case NPCBL_OUT_OF_SPACE:		return 3;
		default:						return 3;
		}
	}

	if(nd->master_nd != NULL) {
		int retval = 0;
		npc_lock_data(nd->master_nd);
		retval = npc->buylist_sub(sd, item_list, nd->master_nd);
	...
	// Pay
	pc->payzeny(sd, (int)out_price, LOG_TYPE_NPC, NULL);
	// Delivery
	for(int i = 0; i < VECTOR_LENGTH(*item_list); ++i) {
		struct itemlist_entry *entry = &VECTOR_INDEX(*item_list, i);
	...
	int idx = skill->get_index(MC_DISCOUNT);
	int skill_t;
	if( battle_config.shop_exp > 0 && out_price > 0 && (skill_t = pc->checkskill2(sd,idx)) > 0 ) {
		if( sd->status.skill[idx].flag >= SKILL_FLAG_REPLACED_LV_0 )
			skill_t = sd->status.skill[idx].flag - SKILL_FLAG_REPLACED_LV_0;

		if( skill_t > 0 ) {
			out_price = apply_percentrate64(out_price, skill_t * battle_config.shop_exp, 10000);
			if(out_price < 1)
				out_price = 1;
			pc->gainexp(sd, NULL, 0, (int)out_price, false);
		}
	}
	npc->unlock_data(nd);
	return 0;
...
static enum market_buy_result npc_market_buylist(struct map_session_data *sd, struct itemlist *item_list)
{
	...
	bool is_invalid = (
		...
		!nd->u.scr.shop || nd->u.scr.shop->type != NST_MARKET ||
		!npc->checknear(sd, nd) // Most process intensive last
		);
	if(is_invalid) {
		npc->unlock_data(nd);
		return MARKET_BUY_RESULT_ERROR;
	...
	// Process validate
	int64 out_price = 0;
	enum npc_buy_result validate_result =
		npc->generic_buylist_process_validate(sd, item_list, nd, shop, shop_size,
		&out_price, sd->status.zeny, BUY_ENFORCE_MAXIMUM_AMOUNT);

	if(validate_result != NPCBL_SUCCESS) {
		npc->unlock_data(nd);
		switch(validate_result) {
		NPCBL_ITEM_NOT_FOUND_SHOP: return MARKET_BUY_RESULT_ERROR;
		NPCBL_ITEM_NOT_FOUND_DB:   return MARKET_BUY_RESULT_ERROR;
		NPCBL_AMOUNT_TOO_BIG:	   return MARKET_BUY_RESULT_AMOUNT_TOO_BIG;
		NPCBL_NO_COIN:			   return MARKET_BUY_RESULT_NO_ZENY;
		NPCBL_OVER_WEIGHT:		   return MARKET_BUY_RESULT_OVER_WEIGHT;
		NPCBL_OUT_OF_SPACE:		   return MARKET_BUY_RESULT_OUT_OF_SPACE;
		default:				   return MARKET_BUY_RESULT_ERROR;
		}
	}

	pc->payzeny(sd, out_price, LOG_TYPE_NPC, NULL);

	for(int i = 0; i < VECTOR_LENGTH(*item_list); ++i) {
		struct itemlist_entry *entry = &VECTOR_INDEX(*item_list, i);

		int j = npc_market_qty[i];
...
/**
 * Processes and validates given barter buy list for a character, then
 * removes items.
 * @param sd                   Player character
 * @param item_list            Player item list to be bought
 * @param nd                   Target NPC
 * @param shop                 Target NPC item list
 * @param shop_size            Target NPC item list length
 * @see npc_barter_buylist
 **/
static int npc_barter_buylist_validate_del(struct map_session_data *sd,
	struct barteritemlist *item_list, struct npc_data *nd,
	struct npc_item_list *shop, unsigned short shop_size
) {
	int w = 0;
	int new_ = 0;
	...
	for(int i = 0; i < VECTOR_LENGTH(*item_list); ++i) {
		...
		if (!itemdb->isstackable(entry->addId) && entry->addAmount > 1) {
			...
			ShowWarning("Player %s (%d:%d) sent a hexed packet trying to buy %d of"
				" non-stackable item %d!\n", 
				sd->status.name, sd->status.account_id, sd->status.char_id,
				entry->addAmount, entry->addId);
			entry->addAmount = 1;
		}
...
static int npc_barter_buylist(struct map_session_data *sd, struct barteritemlist *item_list)
{
	...
	int validate = npc_barter_buylist_validate_del(sd, item_list, nd, shop, shop_size);
	if(validate != 12) {
		npc->unlock_data(nd);
		return validate;
	}
 ...
 * @see npc_barter_buylist
 **/
static int npc_expanded_barter_buylist_validate_del(struct map_session_data *sd,
	struct barteritemlist *item_list, struct npc_data *nd,
	struct npc_item_list *shop, unsigned short shop_size, int64 *out_price
) {
	int w = 0;
...
static int npc_expanded_barter_buylist(struct map_session_data *sd, struct barteritemlist *item_list)
{
	...
	int out_price = 0;
	int validate = npc_expanded_barter_buylist_validate_del(sd, item_list, nd,
		shop, shop_size, &out_price);
	if(validate != 12) {
		npc->unlock_data(nd);
		return validate;
	}
 ...
 * @param item_list            Player item list to be bought
 * @param nd                   Target NPC
 * @param out_profit           Profit (out)
 **/
static int npc_selllist_validate(struct map_session_data *sd,
	struct itemlist *item_list, struct npc_data *nd, int64 *out_profit
)
{
	...
	if (nd->master_nd != NULL) // Script-controlled shops exit early
		return 0;
...
static int npc_selllist(struct map_session_data *sd, struct itemlist *item_list)
{
	...
	if(nd->subtype != SHOP) {
		if(nd->subtype != SCRIPT || nd->u.scr.shop == NULL ||
	...
	// Validate
	int profit = 0;
	if(npc_selllist_validate(sd, item_list, nd, &profit)) {
		npc->unlock_data(nd);
		return 1;
	}

	if (nd->master_nd != NULL) { // Script-controlled shops do all deletion by themselves
		npc->unlock_data(nd);
	...
	pc->getzeny(sd, (int)profit, LOG_TYPE_NPC, NULL);

	int skill_t;
	int skill_idx = skill->get_index(MC_OVERCHARGE);

	// Custom merchant shop exp bonus.
	if (battle_config.shop_exp > 0 && profit > 0 && (skill_t = pc->checkskill2(sd, skill_idx)) > 0) {
		...
		if (skill_t > 0) {
			profit = apply_percentrate64(profit, skill_t * battle_config.shop_exp, 10000);

			if (profit < 1)
				profit = 1;

			pc->gainexp(sd, NULL, 0, (int)profit, false);
		}
	}
 ...
 * Acquires db_lock(npc->ev_label_db)
 *
 * When single is set unlocks npc_data via npc->unlock_data. Otherwise the
 * NPC data mutex must NOT be locked.
 **/
static void npc_unload(struct npc_data *nd, bool single, bool unload_mobs)
{
	...
	if (single) {
		npc->unlock_data(nd);
		db_lock(npc->name_db, WRITE_LOCK);
		strdb_remove(npc->name_db, nd->exname, 0);
		db_unlock(npc->name_db);
		/**
		 * NPC data mutex must be locked even if it's out of name db
		 * because NPCs can be accessed via npc->id2nd.
		 **/
		mutex->lock(nd->lock);
	}
...
/**
 * Parses and sets the name and exname of a npc (validates name).
 * Assumes that m, x and y are already set in nd.
 *
 * @param name     NPC full name ("public::unique")
 * @param line     Line where NPC is declared
 * @param filepath Source file filepath
 *
 * @lock db_lock(npc->name_db)
 * @mutex nd->lock
 **/
static void npc_parsename(struct npc_data *nd, const char *name, const char *filepath, const char *buffer, const char *start)
{
	const char* p;
	...
	if( p ) { // <Display name>::<Unique name>
		size_t len = p-name;
		if( len > NAME_LENGTH ) {
			ShowWarning("npc_parsename: Display name of '%s' is too long (len=%u) "
				"in file '%s', line '%d'. Truncating to %d characters.\n",
		...
		len = strlen(p+2);
		if( len > NAME_LENGTH )
			ShowWarning("npc_parsename: Unique name of '%s' is too long (len=%u) "
				"in file '%s', line '%d'. Truncating to %d characters.\n",
				name, (unsigned int)len, filepath, strline(buffer,start-buffer), NAME_LENGTH);
		...
		if( len > NAME_LENGTH )
			ShowWarning("npc_parsename: Name '%s' is too long (len=%u) in file '%s', "
				"line '%d'. Truncating to %d characters.\n",
				name, (unsigned int)len, filepath, strline(buffer,start-buffer), NAME_LENGTH);
		safestrncpy(nd->name, name, sizeof(nd->name));
	...
	if( *nd->exname == '\0' || strstr(nd->exname,"::") != NULL ) {// invalid
		...
		ShowWarning("npc_parsename: Invalid unique name in file '%s', line '%d'. "
			"Renaming '%s' to '%s'.\n", filepath, strline(buffer,start-buffer), nd->exname, newname);
		safestrncpy(nd->exname, newname, sizeof(nd->exname));
	}

	int name_len = 0;
	if(strdb_get(npc->name_db, nd->exname, 0)) {// duplicate unique name, generate new one
		...
		do {
			++i;
			name_len = safesnprintf(newname, ARRAYLENGTH(newname), "%d_%d_%d_%d",
				i, nd->bl.m, nd->bl.x, nd->bl.y);
		} while(strdb_get(npc->name_db, nd->exname, name_len));

		strcpy(this_mapname,
			(nd->bl.m == -1 ? "(not on a map)" : mapindex_id2name(map_id2index(nd->bl.m))));
		strcpy(other_mapname,
			(dnd->bl.m == -1 ? "(not on a map)" : mapindex_id2name(map_id2index(dnd->bl.m))));

		ShowWarning("npc_parsename: Duplicate unique name in file '%s', line '%d'."
			"Renaming '%s' to '%s'.\n", filepath, strline(buffer,start-buffer), nd->exname, newname);
		ShowDebug("this npc:\n   display name '%s'\n   unique name '%s'\n"
			"\tmap=%s, x=%d, y=%d\n", nd->name, nd->exname, this_mapname, nd->bl.x, nd->bl.y);
		ShowDebug("other npc in '%s' :\n   display name '%s'\n"
			"\tunique name '%s'\n   map=%s, x=%d, y=%d\n",dnd->path, dnd->name, dnd->exname,
			other_mapname, dnd->bl.x, dnd->bl.y);
		safestrncpy(nd->exname, newname, sizeof(nd->exname));
	}
}
...
/**
 * Frees npc data created via npc_create
 *
 * @remarks Any mutexes must be unlocked before destruction.
 **/
static void npc_destroy_npc(struct npc_data *nd)
{
	nullpo_retv(nd);
	VECTOR_CLEAR(nd->qi_data);
	mutex->destroy(nd->lock);
	aFree(nd);
}
 ...
 * @remarks The created NPC is locked and must be unlocked after use.
 */
...
/**
 * Adds and displays an NPC warp on map (from inside the source, not from a script)
 *
 * @param name                   Full NPC name <public>::<unique>
 * @param from_mapid             Origin map
 * @param from_x, xs, from_y, ys Origin area
 * @param to_mapindex            Target map
 * @param to_x, to_y             Target coordinates
 * @return Generated npc data
 * @remarks NPC data lock must be freed via npc->unlock_data
 **/
static struct npc_data *npc_add_warp(char *name, short from_mapid, short from_x,
	short from_y, short xs, short ys, unsigned short to_mapindex, short to_x, short to_y
)
{
	struct npc_data *nd;
	...
	db_lock(npc->name_db, WRITE_LOCK);
	npc->add_to_location(nd, name, "no_path", name, name[0]);
	db_unlock(npc->name_db);
...
static const char *npc_parse_warp(const char *w1, const char *w2, const char *w3, const char *w4, const char *start, const char *buffer, const char *filepath, int *retval)
{
	...
	npc->add_to_location(nd, w3, filepath, buffer, start);
...
static const char *npc_parse_script(const char *w1, const char *w2, const char *w3, const char *w4, const char *start, const char *buffer, const char *filepath, int options, int *retval)
{
	...
	char exname[25];
	memcpy(exname, nd->exname, sizeof(exname));
	mutex->unlock(nd->lock);

	// Execute OnInit
	if( options&NPO_ONINIT )
		npc->event_dolocal(exname, "OnInit", NULL, NULL);
 ...
 * Verifies if the provided NPC name is unique and retains path data.
 *
 * @param nd       NPC to register.
 * @param name     NPC full name ("public::unique")
 * @param filepath Source file filepath
 * @param buffer   Loaded buffer (used for debug purposes)
 * @param start    Where the NPC starts in the given buffer (used for debug purposes)
 *
 * @mutex nd->lock (if not acquired the npc can be accessed before all the loading is finished)
 * Acquires db_lock(npc->name_db)
 */
static void npc_add_to_location(struct npc_data *nd, const char *name, const char *filepath, const char *buffer, const char *start)
{
	nullpo_retv(nd);

	db_lock(npc->name_db, WRITE_LOCK);
	npc->parsename(nd, name, filepath, buffer, start);
	...
	strdb_put(npc->name_db, nd->exname, 0, nd);
	db_unlock(npc->name_db);
}
 ...
 * @readlock npc->name_db, MUST NOT be write lock
 */
...
static const char *npc_parse_duplicate(const char *w1, const char *w2, const char *w3, const char *w4, const char *start, const char *buffer, const char *filepath, int options, int *retval)
{
	...
	dnd = npc->name2id(srcname, 0);
	if( dnd == NULL) {
		ShowError("npc_parse_script: original npc not found for duplicate in file "
	...
	// NPC is added to location inside each of the duplicates
	// npc->add_to_location(nd, w3, filepath, buffer, start);
...
static int npc_duplicate4instance(struct npc_data *snd, int16 m)
{
	...
	if( npc_name_is_valid(newname, (name_len < ARRAYLENGTH(newname))?name_len:0) ) { // Name already in use
		ShowError("npc_duplicate4instance: the npcname (%s) is already in use while "
			"trying to duplicate npc %s in instance %d.\n",
...
static int npc_unsetcells_sub(struct block_list *bl, va_list ap)
{
	...
	if (nd->bl.id == id)
		return 0;
	// Lock is already acquired in npc_unsetcells
	npc->setcells(nd);
	return 1;
}
...
void npc_defaults(void)
{
	...
	npc->lock_name_db   = npc_lock_name_db;
	npc->unlock_name_db = npc_unlock_name_db;
	npc->name2id = npc_name2id;
	npc->id2nd = npc_id2nd;
	npc->bl2nd = npc_bl2nd;
	npc->bl2nd_nodblock = npc_bl2nd_nodblock;
	npc->unlock_data = npc_unlock_data;
	...
	npc->generic_buylist_process_validate = npc_generic_buylist_process_validate;
	...
	npc->destroy_npc = npc_destroy_npc;
