...
struct inter_rodex_interface {
	int (*sql_init) (void);
	void (*sql_final) (void);
	int (*fromsql) (int char_id, int account_id, enum rodex_opentype opentype, int64 mail_id, struct rodex_maillist *mails);
	...
	bool (*updatemail) (struct socket_data *session, int account_id, int char_id, int64 mail_id, uint8 opentype, enum rodex_updatemail_flag flag);
