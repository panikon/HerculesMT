...
struct mapif_interface {
	...
	void (*parse_LoadAchievements) (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*sAchievementsToMap) (struct socket_data *session, int char_id, const struct char_achievements *cp);
	void (*parse_SaveAchievements) (struct s_receive_action_data *act, struct mmo_map_server *server);
	...
	void (*parse_mail_requestinbox) (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*parse_mail_read) (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*mail_sendattach) (struct socket_data *session, int char_id, const struct mail_message *msg);
	void (*parse_mail_getattach) (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*mail_delete) (struct socket_data *session, int char_id, int mail_id, bool failed);
	void (*parse_mail_delete) (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*mail_new) (struct mail_message *msg);
	void (*mail_return) (struct socket_data *session, int char_id, int mail_id, int new_mail);
	void (*parse_mail_return) (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*mail_send) (struct socket_data *session, const struct mail_message* msg);
	void (*parse_mail_send) (struct s_receive_action_data *act, struct mmo_map_server *server);
	...
	void (*parse_mercenary_create) (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*parse_mercenary_load) (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*mercenary_deleted) (struct socket_data *session, int char_id, int merc_id, bool success);
	void (*parse_mercenary_delete) (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*mercenary_saved) (struct socket_data *session, int char_id, int merc_id, bool success);
	void (*parse_mercenary_save) (struct s_receive_action_data *act, struct mmo_map_server *server);
	...
	void (*parse_CreateParty)       (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*parse_PartyInfo)         (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*parse_PartyAddMember)    (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*parse_PartyChangeOption) (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*parse_PartyLeave)        (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*parse_PartyChangeMap)    (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*parse_BreakParty)        (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*parse_PartyLeaderChange) (struct s_receive_action_data *act, struct mmo_map_server *server);
	...
	void (*parse_CreatePet) (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*parse_LoadPet)   (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*parse_SavePet)   (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*parse_DeletePet) (struct s_receive_action_data *act, struct mmo_map_server *server);

	void (*quest_save_ack) (struct socket_data *session, int char_id, bool success);
	void (*parse_quest_save) (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*send_quests) (struct socket_data *session, int char_id, struct quest *quest, int num_quests);
	void (*parse_quest_load) (struct s_receive_action_data *act, struct mmo_map_server *server);

	void (*parse_rodex_requestinbox) (struct s_receive_action_data *act, struct mmo_map_server *server);
	...
	void (*parse_rodex_checkhasnew) (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*rodex_sendhasnew) (struct socket_data *session, int char_id, bool has_new);
	void (*parse_rodex_updatemail) (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*parse_rodex_send) (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*rodex_send) (struct socket_data *session, int sender_id, int receiver_id, int receiver_accountid, bool result);
	void (*parse_rodex_checkname) (struct s_receive_action_data *act, struct mmo_map_server *server);
	...
	void (*parse_LoadGuildStorage) (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*parse_SaveGuildStorage) (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*account_storage_load) (struct socket_data *session, int account_id);
	void (*parse_AccountStorageLoad) (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*parse_AccountStorageSave) (struct s_receive_action_data *act, struct mmo_map_server *server);
	...
	void (*parse_ItemBoundRetrieve) (struct s_receive_action_data *act, struct mmo_map_server *server);

	void (*parse_accinfo) (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*disconnectplayer)  (struct socket_data *session, int account_id, int char_id, enum notify_ban_errorcode reason);
	void (*parse_Registry) (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*parse_RegistryRequest) (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*namechange_ack) (struct socket_data *session, int account_id, int char_id, int type, uint8 flag, const char *esc_name);
	void (*parse_NameChangeRequest) (struct s_receive_action_data *act, struct mmo_map_server *server);
	// Clan System
	void (*ClanMemberKick_ack) (struct socket_data *session, int clan_id, int count);
	void (*parse_ClanMemberKick) (struct s_receive_action_data *act, struct mmo_map_server *server);
	void (*parse_ClanMemberCount) (struct s_receive_action_data *act, struct mmo_map_server *server);
};
